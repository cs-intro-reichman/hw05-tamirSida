...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
	    ...
	    for (int i = 1; i <= rows; i++) {
			String temp = in.readLine(); // Read a line for each row of the board
			if (temp != null) {
				for (int j = 1; j <= temp.length() && j <= cols; j++) {
					board[i][j] = (temp.charAt(j - 1) == 'x') ? 1 : 0; // Convert 'x' to 1, others to 0
				}
			}
		}
		return board;
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		else if (main==1 && (count==2||count==3)){
			main=1;
		}
		else if (main==1 && count>3){
			main=0;
		}
		else if (main==0 && count==3){
			main=1;
		}
		return main;
	}
    ...
    public static void print(int[][] arr) {
		for (int i = 1; i < arr.length - 1; i++) {
			...
			System.out.println(); // Remove the extra line here
		}
	}
